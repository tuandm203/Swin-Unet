...
def trainer_synapse_v2(args, model, snapshot_path):
    from datasets.dataset_synapse_v2 import Synapse_dataset, RandomGenerator
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in tqdm(enumerate(train_loader), desc=f"Train: {epoch_num}", total=len(train_loader),
            ...
            # 1. BẮT BUỘC PHẢI CÓ DÒNG NÀY TRƯỚC ĐỂ TẠO BIẾN
            image_batch, label_batch = sampled_batch['image'], sampled_batch['label']
            
            # === ĐOẠN CODE CHÈN THÊM ĐỂ CHECK 2.5D ===
            if i_batch == 0:  # Chỉ in ở mẻ đầu tiên để đỡ trôi màn hình
                print(f"\n--- KIỂM TRA ĐẦU VÀO ---")
                print(f"Shape của ảnh: {image_batch.shape}")
                print(f"Shape của nhãn: {label_batch.shape}")
                print(f"------------------------\n")
            # ========================================
            image_batch, label_batch = sampled_batch['image'], sampled_batch['label']
